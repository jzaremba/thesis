...
\todo{Attila highlighted above sentence, not sure why?}
...
Spatial working memory is the short-term, on-line processing of spatial information and requires both the \ac{HPC} to provide spatial maps \citep{Morris1982} as well as the \ac{PFC} to maintain these representation in working memory \citep{XXXX, Olton1979}.
...
\todo[color=cyan, inline]{define annular water maze? Jeff: not exactly sure what in particular needs to be defined}
...
\todo[color=green]{General comment to check HPC subfield abbreviations}
...
\subsection{Functional organization of the hippocampus}
...
% \todo[color=cyan]{REF: Chembrowski, Thompson papers}

\subsubsection{Pyramidal cell functional diversity}
\label{sec:into:memory:diversity}
...
\todo[color=cyan]{REFS -- Mizuseki, etc.}
...
\todo[color=cyan]{Expand on CA1PC subdivisions/heterogeneity}
...
Finally, the CA1 output node of the hippocampus has been suggested to play a role in novelty detection as a \textsc{comparator} \citep{Vinogradova2001, Lisman2001}, as it has access to both the internal \ac{HPC} processed information from CA3 Schaffer collaterals as well as direct perforant path excitation from the \ac{EC} (\autoref{fig:intro:memory:subfield_IO}c).
...
In particular, the perforant path inputs to CA1 include both \ac{MEC} and \ac{LEC} projections which convey two distinct streams of information.
...
These inputs streams are not completely spatially separated, as within place cell populations differences in locations are encoded through global remapping and differences in context are encoded through rate remapping (see \autoref{sec:intro:memory:remapping}, \citet{Leutgeb2005a}).
...
\todo[inline, color=cyan]{NMDAR/LTP, Tonegawa NMDAR KO mouse}
...
Within the \ac{EC}, principal cells take on a variety of spatial firing properties, but one of the main categories are cells that fire at regularly spaced intervals throughout an environment (\textsc{grid cells}) \citep{Hafting2005, Moser2014a}.
\todo[color=cyan]{ -- MEC, mostly layer II -- add more details and reviews. Border cells, head-direction cells?}
...
This long-term stability of place fields is both NMDA receptors-dependent \citep{Kentros1998} and requires new protein synthesis \citep{Agnihotri2004}, in a similar manner as LTP and other forms of declarative memory \citep{Frey1988, Hernandez2008}.
