...
In vivo Ca{2+} imaging provides several essential advantages over \emph{in vivo} electophysilogical approahces which I made use of in my thesis work.
...
Second, registering fields of view from day-to-day let me reliably track the same cells for up to 2~weeks. The main experimental task that I used (\nameref{sec:intro:techniques:GOL}) was a 9-day protocol and tracking the same cells across those 9 days was essential.
...
\subsection{Place cell identification}\label{sec:intro:techniques:pc_id}
...
\subsection{Place cell stability}\label{sec:intro:techniques:stability}
...
\textsc{Population vector (PV) correlation} is similar to PF correlation but measures the correlation across cells at each spatial bin, instead of the correlation across spatial bins for each cell. This metric is calculated by `stacking' place field tuning curves for each cell, which 
\todo[inline, color=yellow]{Finish describing stability metrics.}
